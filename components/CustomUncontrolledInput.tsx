...
const CustomUncontrolledInput = ({ title = "CustomUncontrolledInput", type, color, name, required = false }: Props) => {
    ...
    return (
        <div
            className={`flex flex-auto py-2 pl-5 pr-2 rounded-md border-gray-400 border-2 focus-within:border-black hover:focus-within:border-${color} relative group z-10`}
        >
            <input
                type={type || 'text'}
                name={name}
